...
import { buyUpgrade, buyBuildingUpgrade, changeCash, changeLevel, planetConfigsList } from '../store/index';
...
export default function PixelUpgradeModal({ open, onClose, gameEnded, setGameEnded }) {
  ...
  const planetConfig = planetConfigsList[levelNumber];

  const upgradePrices = planetConfig.upgradePrices;
  const cashPerClickBonuses = planetConfig.clickBonuses;
  const cashPerSecondMultipliers = planetConfig.perSecondMultipliers;
  const buildingBasePerSecond = planetConfig.buildingBasePerSecond;
  ...
  const getBuildingUpgradePrice = (house, nextLevel) => {
    const prices = planetBuildingPrices[planetConfig.name]?.[house];
    if (!prices) return 0;
    return prices[nextLevel - 1] || 0;
  };
        ...
        <div style={{
          ...
          <div style={{
            ...
            {Object.entries(upgrades.building_upgrades).map(([house, { level }]) => {
              ...
              const price = getBuildingUpgradePrice(house, nextLevel);
